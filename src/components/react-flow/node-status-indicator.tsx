...
const StatusBorder = ({
  ...
  return (
    <>
      <div
        className={cn(
          "absolute -top-[1.5px] -left-[1.5px] h-[calc(100%+3px)] w-[calc(100%+3px)] rounded-[6px] border-4",
          className,
        )}
      />
      {children}
...
export const NodeStatusIndicator = ({
  ...
  switch (status) {
    ...
    case "success":
      return (
        <StatusBorder className="border-green-700/60">{children}</StatusBorder>
      );
    case "error":
      return (
        <StatusBorder className="border-red-700/50">{children}</StatusBorder>
      );
    default:
      return <>{children}</>;
  }
